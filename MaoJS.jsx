$(document).ready(function(){
    ...
    $('#btxt').animate({width: "0",height: "0"},100,function(){
    $('#btxt').html(mytextshow);
    $('#btxt').animate({width: "400",height: "20"},1000);});
        timer = setInterval(rant,5000);});
    ranit();
    function ranit(){
    myx = new Date();
    y=Math.floor(Math.random()*myrindex)
        //y = myx.getSconds()% myrindex;
    myshow=myrand[y]
    mytextshow=$(myshow).attr("title");
    $('#banner').animate({width: "0",height: "60"},100,function(){
    $('#banner').html(myshow);
